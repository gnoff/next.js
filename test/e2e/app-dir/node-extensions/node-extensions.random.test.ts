  ...
  describe('Random', () => {
    describe('Dynamic IO', () => {
      ...
      it('should not error when accessing routes that use Math.random() in App Router', async () => {
        ...
        expect(secondBody.rand1).not.toBe(secondBody.rand2)
        expect(secondBody.rand1).not.toBe(body.rand1)
        expect(secondBody.rand2).not.toBe(body.rand2)
        ...
        expect(body.rand1).toBe(body.rand2)
        res = await next.fetch('/app/prerendered/unstable-cache/api')
        secondBody = await res.json()
        expect(secondBody.rand1).toBe(secondBody.rand2)
        expect(secondBody.rand1).toBe(body.rand1)
        expect(secondBody.rand2).toBe(body.rand2)
        ...
        res = await next.fetch('/app/rendered/use-cache/api')
        secondBody = await res.json()
        // TODO use-cache doesn't guarantee cached for dynamic cache reads so we don't assert here
        // that the second read actually matches the first.
    ...
    describe('Legacy', () => {
      ...
      it('should not error when accessing routes that use Math.random() in App Router', async () => {
        ...
        secondBody = await res.json()
        expect(secondBody.rand1).not.toBe(secondBody.rand2)
        // In legacy mode when GET handlers are cached the same response should be returned
        expect(secondBody.rand1).toBe(body.rand1)
        expect(secondBody.rand2).toBe(body.rand2)
